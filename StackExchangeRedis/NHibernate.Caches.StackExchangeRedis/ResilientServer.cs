...
namespace NHibernate.Caches.StackExchangeRedis
{
	internal class ResilientServer : IServer
	{
		...
		public ResilientServer(IResilientConnectionMultiplexer resilientConnectionMultiplexer, Func<IServer> serverProvider)
		{
		...
		/// <inheritdoc />
		public bool AllowSlaveWrites { get => _server.Value.AllowSlaveWrites; set => _server.Value.AllowSlaveWrites = value; }
		...
		/// <inheritdoc />
		public IEnumerable<RedisKey> Keys(int database = 0, RedisValue pattern = default, int pageSize = 10, long cursor = 0, int pageOffset = 0, CommandFlags flags = CommandFlags.None)
		{
